...
import funkin.visuals.cutscenes.DialogueBox;
...
class SchoolEvil extends BaseStage
{
	override function create()
	{
		...
		if(!ClientPrefs.data.lowQuality)
			bg = new BGSprite('week6/weeb/animatedEvilSchool', posX, posY, 0.8, 0.9, ['background 2'], true);
		else
			bg = new BGSprite('week6/weeb/animatedEvilSchool_low', posX, posY, 0.8, 0.9);
	...
	override function eventPushed(event:funkin.visuals.game.Note.EventNote)
	{
	...
	function initDoof()
	{
		var file:String = Paths.getPath('data/' + songName + '/' + songName + 'Dialogue.txt'); //Checks for vanilla/Senpai dialogue
		#if MODS_ALLOWED
